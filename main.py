...
version=" "
def printcolor(text):
    print(Colorate.Horizontal(Colors.blue_to_cyan,text,1))
...
while True:
    ...
    printcolor(version)
    print()
    printcolor("Bot Token:")
    token=input(Colorate.Horizontal(Colors.blue_to_cyan,">",1))
    ...
    if tokenverify.status_code==200:
        while True:
            ...
            printcolor(version)
            print()
            printcolor(" Server Id:")
            guildid=input(Colorate.Horizontal(Colors.blue_to_cyan,">",1))
...
while True:
    ...
    printcolor("[5] Rename Channels         [6] Rename Roles            [7] Rename All          [8] Rename Emojis")
    printcolor("[9] Kick all                [10] Ban all                [11] Unban all          [12] Massege Spammer")
    printcolor("[14] Dm Spam")
    print()
    printcolor("")
    printcolor("")
    cmd=input(Colorate.Horizontal(Colors.blue_to_cyan,">",1))
    if cmd=="1":
        #create channels
        printcolor("Name of channel:")
        new=input(Colorate.Horizontal(Colors.blue_to_cyan,">",1))
        printcolor("Channel type:")
        printcolor("text/voice/category")
        new1=input(Colorate.Horizontal(Colors.blue_to_cyan,">",1))
        ...
        if cango==1:
            ...
            repeat=int(input(Colorate.Horizontal(Colors.blue_to_cyan,">",1)))
        ...
        new=int(input(Colorate.Horizontal(Colors.blue_to_cyan,">",1)))
        api = requests.get(f"https://discord.com/api/v9/guilds/{guildid}/channels", headers={"Authorization": f"Bot {token}"})
        for channel in api.json():
            count=0
            while not count==new:
